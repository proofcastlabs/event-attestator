...
fn get_sample_submission_material_string_n(chain_type: &str, n: usize) -> Result<String> {
    let path = format!("src/test_utils/{chain_type}-submission-material-{n}.json");
    if Path::new(&path).exists() {
        Ok(read_to_string(path)?)
    } else {
        Err(format!(
            "✘ Can't find sample {} submission material #{n} file!",
...
pub fn get_sample_eth_evm_dictionary() -> EthEvmTokenDictionary {
    EthEvmTokenDictionary::from_str(&read_to_string("src/test_utils/eth-evm-sample-dictionary.json").unwrap()).unwrap()
}
