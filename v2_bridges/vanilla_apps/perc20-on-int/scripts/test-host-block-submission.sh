...
getCoreLatestHostBlockNumbers() {
	$BINARY_PATH getEnclaveState | jq .int.evm_latest_block_number
}
...
if [[ $(getCoreLatestHostBlockNumbers) == null ]];then
	../../scripts/clean-up.sh $BINARY_NAME
...
[[ $(getCoreLatestHostBlockNumbers) == $(getExpectedBlockNumber) ]] && result=true || result=false
