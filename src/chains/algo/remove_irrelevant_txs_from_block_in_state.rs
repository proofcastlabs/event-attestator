...
pub fn remove_irrelevant_txs_from_block_in_state<D: DatabaseInterface>(state: AlgoState<D>) -> Result<AlgoState<D>> {
    ...
    remove_irrelevant_txs_from_block(&block, state.get_evm_algo_token_dictionary()?.to_algo_asset_ids()).and_then(
        |relevant_txs| {
            block.transactions = if relevant_txs.is_empty() {
                None
            } else {
                Some(relevant_txs)
            };
...
mod tests {
    ...
    #[test]
    fn should_remove_all_txs_from_block_in_state_if_none_are_relevant() {
        ...
        let state_3 = state_2.add_submitted_algo_block(&block).unwrap();
        assert!(state_3.get_submitted_algo_block().unwrap().transactions.unwrap().len() > 0);
        let state_4 = remove_irrelevant_txs_from_block_in_state(state_3).unwrap();
        let result = state_4.get_submitted_algo_block().unwrap();
    ...
    fn should_remove_irrelevant_txs_from_block_in_state() {
        ...
        let db = get_test_database();
        let block = get_sample_block_n(0);
        let num_txs_before = block.transactions.clone().unwrap().len();
        let state_1 = AlgoState::init(&db);
        let state_2 = state_1.add_evm_algo_dictionary(dict).unwrap();
        let state_3 = state_2.add_submitted_algo_block(&block).unwrap();
        assert!(state_3.get_submitted_algo_block().unwrap().transactions.unwrap().len() > 0);
        let state_4 = remove_irrelevant_txs_from_block_in_state(state_3).unwrap();
        let num_txs_after = state_4.get_submitted_algo_block().unwrap().transactions.unwrap().len();
