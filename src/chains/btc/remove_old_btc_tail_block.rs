...
use crate::{
    chains::btc::{
        ...
        btc_database_utils::{get_btc_anchor_block_hash_from_db, get_btc_block_from_db, get_btc_tail_block_from_db},
        btc_state::BtcState,
    },
...
fn is_anchor_block<D: DatabaseInterface>(db: &D, btc_block_hash: &BlockHash) -> Result<bool> {
...
fn remove_parents_if_not_anchor<D>(db: &D, block_whose_parents_to_be_removed: &BtcBlockInDbFormat) -> Result<()>
where
    D: DatabaseInterface,
{
        ...
        Ok(parent_block) => {
            ...
            match is_anchor_block(db, &parent_block.id)? {
                ...
                false => {
                    ...
                    db.delete(parent_block.id.to_vec())
...
mod tests {
    ...
    fn should_return_false_block_is_not_anchor_block() {
        ...
        let result = is_anchor_block(&db, &non_anchor_block.id).unwrap();
        assert!(!result);
    }
    ...
    fn should_return_true_if_block_is_anchor_block() {
        ...
        let result = is_anchor_block(&db, &anchor_block.id).unwrap();
        assert!(result);
    }
    ...
    fn should_remove_parent_blocks_recursively_if_not_anchor_blocks() {
        ...
        if let Err(e) = remove_parents_if_not_anchor(&db, &tail_block) {
            panic!("Error removing parent block if not anchor: {}", e);
        };
        blocks
            .iter()
