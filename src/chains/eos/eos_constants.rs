...
pub fn get_eos_constants_db_keys() -> JsonValue {
    json!({
        "EOS_CHAIN_ID_DB_KEY":
            hex::encode(EOS_CHAIN_ID_DB_KEY.to_vec()),
        "EOS_PRIVATE_KEY_DB_KEY":
            hex::encode(EOS_PRIVATE_KEY_DB_KEY.to_vec()),
        "PROCESSED_TX_IDS_KEY":
            hex::encode(PROCESSED_TX_IDS_KEY.to_vec()),
        "EOS_ACCOUNT_NAME_KEY":
            hex::encode(EOS_ACCOUNT_NAME_KEY.to_vec()),
        "EOS_TOKEN_SYMBOL_KEY":
            hex::encode(EOS_TOKEN_SYMBOL_KEY.to_vec()),
        "EOS_ACCOUNT_NONCE":
            hex::encode(EOS_ACCOUNT_NONCE.to_vec()),
        "EOS_SCHEDULE_LIST":
            hex::encode(EOS_SCHEDULE_LIST.to_vec()),
        "EOS_INCREMERKLE":
            hex::encode(EOS_INCREMERKLE.to_vec()),
        "EOS_LAST_SEEN_BLOCK_ID":
            hex::encode(EOS_LAST_SEEN_BLOCK_ID.to_vec()),
        "EOS_LAST_SEEN_BLOCK_NUM":
            hex::encode(EOS_LAST_SEEN_BLOCK_NUM.to_vec()),
        "EOS_PROTOCOL_FEATURES":
            hex::encode(EOS_PROTOCOL_FEATURES.to_vec()),
    })
}

lazy_static! {
    pub static ref EOS_CHAIN_ID_DB_KEY: [u8; 32] = get_prefixed_db_key(
        "eos-chain-id-key"
    );
}
