...
use crate::{
    chains::eos::{eos_database_utils::EOS_PROCESSED_TX_IDS_KEY, eos_state::EosState},
...
impl ProcessedGlobalSequences {
    ...
    pub fn get_from_db<D: DatabaseInterface>(db: &D) -> Result<Self> {
        ...
        db.get(EOS_PROCESSED_TX_IDS_KEY.to_vec(), MIN_DATA_SENSITIVITY_LEVEL)
    ...
    pub fn put_in_db<D: DatabaseInterface>(&self, db: &D) -> Result<()> {
        ...
        db.put(
            EOS_PROCESSED_TX_IDS_KEY.to_vec(),
            self.to_bytes()?,
