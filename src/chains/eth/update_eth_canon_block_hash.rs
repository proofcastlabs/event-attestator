...
fn does_canon_block_require_updating<D: DatabaseInterface>(
    ...
    eth_db_utils.get_eth_canon_block_from_db().and_then(|canon_block| {
        Ok(canon_block.get_block_number()? < calculated_canon_block_and_receipts.get_block_number()?)
    })
}

fn maybe_get_nth_ancestor_of_latest_block<D: DatabaseInterface>(
    eth_db_utils: &EthDatabaseUtils<D>,
    n: u64,
) -> Result<Option<EthSubmissionMaterial>> {
    info!("✔ Maybe getting ancestor #{} of latest ETH block...", n);
    match eth_db_utils.get_eth_latest_block_from_db() {
        Ok(submission_material) => {
            eth_db_utils.maybe_get_nth_ancestor_eth_submission_material(&submission_material.get_block_hash()?, n)
        },
        Err(_) => Ok(None),
    }
}

pub fn maybe_update_canon_block_hash<D: DatabaseInterface>(
    eth_db_utils: &EthDatabaseUtils<D>,
    canon_to_tip_length: u64,
) -> Result<()> {
    match maybe_get_nth_ancestor_of_latest_block(eth_db_utils, canon_to_tip_length)? {
        None => {
        ...
        Some(ancestor_block) => {
            ...
            match does_canon_block_require_updating(eth_db_utils, &ancestor_block)? {
                true => {
                    info!("✔ Updating canon block...");
                    eth_db_utils.put_eth_canon_block_hash_in_db(&ancestor_block.get_block_hash()?)
                },
                false => {
