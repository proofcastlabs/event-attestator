...
fn does_tail_block_require_updating<D>(db: &D, calculated_tail_block: &EthSubmissionMaterial) -> Result<bool>
where
    D: DatabaseInterface,
{
...
pub fn maybe_update_eth_tail_block_hash<D>(db: &D) -> Result<()>
where
    D: DatabaseInterface,
{
...
pub fn maybe_update_eth_tail_block_hash_and_return_state<D>(state: EthState<D>) -> Result<EthState<D>>
where
    D: DatabaseInterface,
{
    info!("✔ Maybe updating ETH tail block hash...");
