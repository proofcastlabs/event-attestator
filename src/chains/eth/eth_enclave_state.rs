...
mod tests {
    ...
    fn test_enclave_state<D: DatabaseInterface, E: EthDbUtilsExt<D>>(db_utils: &E) {
        ...
        if db_utils.get_is_for_eth() {
            ...
            let result = serde_json::to_string(&EthEnclaveState::new(db_utils, &eth_address, router_address).unwrap()).unwrap();
            ...
            let result = serde_json::to_string(&EvmEnclaveState::new(db_utils, &eth_address, router_address).unwrap()).unwrap();
            assert_eq!(result, expected_result);
        }
    }
