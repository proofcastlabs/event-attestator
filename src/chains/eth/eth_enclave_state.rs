        ...
        paste! {
            ...
            impl $name {
                pub fn new<D: DatabaseInterface, E: EthDbUtilsExt<D>>(
                    ...
                    Ok(Self {
                        [<$prefix _tail_length>]: ETH_TAIL_LENGTH,
                        [<$prefix _gas_price>]: db_utils.get_eth_gas_price_from_db()?,
                        [<$prefix _safe_address>]: hex::encode([<SAFE_ $prefix:upper _ADDRESS>].as_bytes()),
                        any_sender_nonce: db_utils.get_any_sender_nonce_from_db()?,
                        [<$prefix _chain_id>]: db_utils.get_eth_chain_id_from_db()?.to_u8(),
                        [<$prefix _account_nonce>]: db_utils.get_eth_account_nonce_from_db()?,
                        ...
                        [<$prefix _tail_block_hash>]: hex::encode([<$prefix _tail_block>].get_block_hash()?.as_bytes()),
                        [<$prefix _address>]: hex::encode(db_utils.get_public_eth_address_from_db()?.as_bytes()),
                        [<$prefix _canon_block_hash>]: hex::encode([<$prefix _canon_block>].get_block_hash()?.as_bytes()),
                        [<$prefix _anchor_block_hash>]: hex::encode([<$prefix _anchor_block>].get_block_hash()?.as_bytes()),
                        [<$prefix _latest_block_hash>]: hex::encode([<$prefix _latest_block>].get_block_hash()?.as_bytes()),
...
mod tests {
    ...
    fn test_enclave_state<D: DatabaseInterface, E: EthDbUtilsExt<D>>(db_utils: &E) {
        ...
        if db_utils.get_is_for_eth() {
            ...
            let result = serde_json::to_string(&EthEnclaveState::new(db_utils, &eth_address).unwrap()).unwrap();
            ...
            let result = serde_json::to_string(&EvmEnclaveState::new(db_utils, &eth_address).unwrap()).unwrap();
            assert_eq!(result, expected_result);
        }
    }
