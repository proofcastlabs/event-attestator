...
pub fn get_linker_hash_or_genesis_hash<D: DatabaseInterface>(eth_db_utils: &EthDatabaseUtils<D>) -> Result<EthHash> {
    match eth_db_utils.get_hash_from_db_via_hash_key(EthHash::from_slice(&ETH_LINKER_HASH_KEY[..]))? {
        Some(hash) => Ok(hash),
        None => {
