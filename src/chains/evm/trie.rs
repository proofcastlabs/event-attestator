...
impl Trie {
    ...
    pub fn put(self, key: Nibbles, value: Bytes) -> Result<Self> {
        ...
        match self.root == HASHED_NULL_NODE {
            true => {
                ...
                Node::new_leaf(key, value).and_then(|leaf| self.update_trie_trie_hash_map(vec![leaf], Vec::new()))
            },
            false => {
                trace!("✔ Trie not-empty ∴ finding nearest node to key...");
                Trie::find(self, key)
                    .and_then(|(self_, target_key, found_stack, remaining_key)| {
                        self_.process_found_node_stack(target_key, found_stack, remaining_key, value)
                    })
                    .and_then(|(self_, target_key, old_stack, new_stack, stack_to_delete)| {
                        self_.update_stale_nodes(target_key, old_stack, new_stack, stack_to_delete)
                    })
                    .and_then(|(self_, _, _, new_stack, stack_to_delete)| {
                        self_.update_trie_trie_hash_map(new_stack, stack_to_delete)
                    })
            },
        }
    }
    ...
    fn process_found_node_stack(
        ...
        match found_stack.pop() {
            Some(node) => match node.get_type() {
                "leaf" => self.process_from_leaf_node(target_key, node, found_stack, remaining_key, value),
                "branch" => self.process_from_branch_node(target_key, node, found_stack, remaining_key, value),
                "extension" => self.process_from_extension_node(target_key, node, found_stack, remaining_key, value),
    ...
    fn process_from_extension_node(
        ...
        get_common_prefix_nibbles(remaining_key, current_ext_node.get_key()).and_then(
            |(common_prefix, key_remainder, node_key_remainder)| match common_prefix.len() {
                0 => match node_key_remainder.len() {
                    1 => split_at_first_nibble(&node_key_remainder).and_then(|(ext_first_nibble, _)| {
                        ...
                        let new_leaf = Node::new_leaf(key_remainder_nibbles, value)?;
                ...
                _ => match node_key_remainder.len() {
                    1 => {
                        ...
                        let (key_remainder_first_nibble, key_remainder_nibbles) =
                            split_at_first_nibble(&key_remainder)?;
                        let (node_key_remainder_first_nibble, _) = split_at_first_nibble(&node_key_remainder)?;
                        let new_leaf = Node::new_leaf(key_remainder_nibbles, value)?;
                        let new_branch = Node::new_branch(None)?;
                        let updated_new_branch = new_branch.update_branch_at_index(
                            Some(convert_h256_to_bytes(new_leaf.get_hash()?)),
                            convert_nibble_to_usize(key_remainder_first_nibble),
                        )?;
                        let final_new_branch = updated_new_branch.update_branch_at_index(
                            Some(
                                current_ext_node
                                    .get_value()
                                    .ok_or(NoneError("Could not unwrap the value of current ext node!"))?,
                        ...
                        let shorter_ext =
                            Node::new_extension(common_prefix, convert_h256_to_bytes(final_new_branch.get_hash()?))?;
                    ...
                    _ => {
                        ...
                        let ext_above_branch = Node::new_extension(common_prefix, final_branch_hash)?;
                        new_stack.push(new_leaf);
    ...
    fn process_from_leaf_node(
        ...
        match remaining_key.len() {
            0 => Node::new_leaf(current_leaf_node.get_key(), value).map(|new_leaf| {
                trace!("✔ No key remaining ∴ creating new leaf node");
                new_stack.push(new_leaf);
                (self, target_key, found_stack, new_stack, Vec::new())
            }),
            _ => get_common_prefix_nibbles(remaining_key, current_leaf_node.get_key()).and_then(
                |(common_prefix, key_remainder, node_key_remainder)| match common_prefix.len() {
                    0 => split_at_first_nibble(&node_key_remainder).and_then(|(leaf_first_nibble, leaf_nibbles)| {
                        ...
                        let new_leaf_2 = Node::new_leaf(key_remainder_nibbles, value)?;
                        let new_branch = Node::new_branch(None)?;
                    ...
                    _ => split_at_first_nibble(&node_key_remainder).and_then(|(leaf_first_nibble, leaf_nibbles)| {
                        ...
                        let updated_branch_hash = convert_h256_to_bytes(updated_branch.get_hash()?);
                        let new_extension = Node::new_extension(common_prefix, updated_branch_hash)?;
    ...
    fn process_from_branch_node(
        ...
        split_at_first_nibble(&remaining_key)
            .and_then(|(first_nibble, remaining_nibbles)| {
                ...
                let new_leaf = Node::new_leaf(remaining_nibbles, value)?;
    ...
    fn update_nodes_from_old_extension_node(
        ...
        let updated_extension_node =
            Node::new_extension(current_node.get_key(), convert_h256_to_bytes(target_node_hash))?;
    ...
    fn update_nodes_from_old_branch_node(
        ...
        trace!("✔ Updating branch index {} to: {}", branch_index, target_node_hash);
    ...
    /// root.
    fn update_trie_trie_hash_map(self, mut new_stack: NodeStack, mut stack_to_delete: NodeStack) -> Result<Self> {
        match !stack_to_delete.is_empty() {
            true => {
                ...
                self.remove_node_from_trie_hash_map(node)
                    .and_then(|new_self| new_self.update_trie_trie_hash_map(new_stack, stack_to_delete))
            },
            false => match new_stack.len() {
                ...
                1 => {
                    ...
                    self.put_node_in_trie_hash_map(node).and_then(|new_self| {
    ...
    pub fn find(self, target_key: Nibbles) -> Result<(Self, Nibbles, NodeStack, Nibbles)> {
        get_node_from_trie_hash_map(&self.trie_hash_map, &self.root).and_then(|maybe_node| match maybe_node {
    ...
    fn find_path(
        ...
        match found_stack.pop() {
            ...
            Some(current_node) => match current_node.get_type() {
                "leaf" => Self::continue_finding_from_leaf(self, target_key, current_node, found_stack, remaining_key),
                "branch" => {
                    Self::continue_finding_from_branch(self, target_key, current_node, found_stack, remaining_key)
                },
                "extension" => {
                    Self::continue_finding_from_extension(self, target_key, current_node, found_stack, remaining_key)
                },
                ...
                match common_prefix.len() {
                    ...
                    _ => match !remaining_node_key.is_empty() {
                        ...
                        false => {
                            ...
                            match get_node_from_trie_hash_map(&self.trie_hash_map, next_node_hash)? {
                                Some(next_node) => {
    ...
    fn continue_finding_from_branch(
        ...
        split_at_first_nibble(&key).and_then(|(first_nibble, remaining_nibbles)| {
            match &branch_node
                .branch
                .ok_or(NoneError("Could not unwrap the branch!"))?
    ...
    fn put_node_in_trie_hash_map(self, node: Node) -> Result<Self> {
        Ok(Trie {
            root: self.root,
            trie_hash_map: put_thing_in_trie_hash_map(self.trie_hash_map, node.get_hash()?, node.get_rlp_encoding()?)?,
        })
    }
    ...
    fn remove_node_from_trie_hash_map(self, node: Node) -> Result<Self> {
        Ok(Trie {
            root: self.root,
            trie_hash_map: remove_thing_from_trie_hash_map(self.trie_hash_map, &node.get_hash()?)?,
        })
    }
}
...
pub fn put_in_trie_recursively(trie: Trie, key_value_tuples: Vec<(Nibbles, Bytes)>, i: usize) -> Result<Trie> {
