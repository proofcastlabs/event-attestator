use crate::{
    chains::{
        eos::{
            core_initialization::check_eos_core_is_initialized::is_eos_core_initialized,
...
pub fn check_core_is_initialized<D: DatabaseInterface>(
    ...
    info!("✔ Checking `INT-on-EOS` core is initialized...");
    if !is_eos_core_initialized(eos_db_utils) {
        return Err("EOS core not initialized!".into());
    }
    if !is_eth_core_initialized(eth_db_utils) {
        return Err("INT` core not initialized!".into());
    }
    Ok(())
}
