...
pub fn debug_get_erc20_vault_migration_tx<D: DatabaseInterface>(
    db: D,
    new_vault_contract_address_str: &str,
) -> Result<String> {
    db.start_transaction()?;
    info!("✔ Debug getting migration transaction...");
    let eth_db_utils = EthDbUtils::new(&db);
    let current_eth_account_nonce = eth_db_utils.get_eth_account_nonce_from_db()?;
    let current_vault_contract_address = eth_db_utils.get_int_on_eos_smart_contract_address_from_db()?;
    let new_vault_contract_address = get_eth_address_from_str(new_vault_contract_address_str)?;
    check_debug_mode()
        ...
        .and_then(|_| eth_db_utils.put_erc20_on_eos_smart_contract_address_in_db(&new_vault_contract_address))
        .and_then(|_| encode_erc20_vault_migrate_fxn_data(new_vault_contract_address))
        .and_then(|tx_data| {
            ...
            Ok(EthTransaction::new_unsigned(
                ...
                0,
                current_vault_contract_address,
                &chain_id,
        ...
        .and_then(|hex_tx| {
            ...
            Ok(json!({
                ...
                "migrated_to_address:": new_vault_contract_address_str.to_string(),
            })
            .to_string())
        })
}
