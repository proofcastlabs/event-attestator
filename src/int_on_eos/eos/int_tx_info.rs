...
/*
 *
impl FeeCalculator for IntOnEosIntTxInfo {
    fn get_amount(&self) -> U256 {
        info!("✔ Getting token amount in `IntOnEosIntTxInfo` of {}", self.amount);
        self.amount
    }
    ...
    fn subtract_amount(&self, subtrahend: U256) -> Result<Self> {
        if subtrahend >= self.amount {
            Err("Cannot subtract amount from `IntOnEosIntTxInfo`: subtrahend too large!".into())
        } else {
            let new_amount = self.amount - subtrahend;
            debug!(
                "Subtracting {} from {} to get final amount of {} in `IntOnEosIntTxInfo`!",
...
impl FeesCalculator for IntOnEosIntTxInfos {
    fn get_fees(&self, dictionary: &EosEthTokenDictionary) -> Result<Vec<(EthAddress, U256)>> {
        debug!("Calculating fees in `IntOnEosIntTxInfos`...");
        self.iter()
...
impl IntOnEosIntTxInfo {
    fn get_memo_string_from_proof(proof: &EosActionProof) -> Result<String> {
        proof
            .check_proof_action_data_length(25, "Not enough data to parse `IntOnEosIntTxInfo` memo from proof!")
...
mod tests {
    ...
    fn get_sample_erc20_on_eos_redeem_infos() -> IntOnEosIntTxInfos {
        IntOnEosIntTxInfos::new(vec![
    ...
    fn should_get_erc20_on_eos_eth_redeem_amount() {
        ...
        let result = IntOnEosIntTxInfo::get_redeem_amount_from_proof(&proof, &dictionary_entry).unwrap();
    ...
    fn should_get_erc20_on_eos_eth_redeem_address() {
        ...
        let proof = get_sample_action_proof_for_erc20_redeem();
        let result = IntOnEosIntTxInfo::get_redeem_address_from_proof_or_default_to_safe_address(&proof).unwrap();
        assert_eq!(result, expected_result);
    }
    ...
    fn should_convert_proof_to_erc20_on_eos_redeem_info() {
        ...
        let result =
            IntOnEosIntTxInfo::from_action_proof(&proof, &dictionary, &origin_chain_id, &vault_address).unwrap();
        assert_eq!(result, expected_result);
    }
    ...
    fn should_fail_to_subtract_too_large_amount_from_erc20_on_eos_redeem_info() {
        let info = get_sample_erc20_on_eos_redeem_info();
        let expected_err = "Cannot subtract amount from `IntOnEosIntTxInfo`: subtrahend too large!".to_string();
