...
impl IntOnEosEosTxInfos {
    fn is_log_erc20_vault_peg_in(log: &EthLog) -> bool {
        log.contains_topic(&ERC20_VAULT_PEG_IN_EVENT_WITHOUT_USER_DATA_TOPIC)
...
mod tests {
    ...
    fn log_is_supported_erc20_peg_in_should_be_true_if_supported() {
        ...
        let result = IntOnEosEosTxInfos::is_log_supported_erc20_peg_in(&log, &dictionary).unwrap();
        assert!(result);
    }
    ...
    fn should_get_get_erc20_redeem_infos_from_receipt() {
        ...
        let result = IntOnEosEosTxInfos::from_eth_receipt(&receipt, &dictionary, &origin_chain_id).unwrap();
    ...
    fn should_filter_submission_material_for_receipts_containing_supported_erc20_peg_ins() {
        ...
        let filtered_submission_material =
            IntOnEosEosTxInfos::filter_eth_sub_mat_for_supported_peg_ins(&submission_material, &dictionary)
                .unwrap();
    ...
    fn should_get_erc20_on_eos_peg_in_infos() {
        ...
        let result =
            IntOnEosEosTxInfos::from_submission_material(&submission_material, &dictionary, &origin_chain_id)
                .unwrap();
        assert_eq!(result.len(), expected_num_results);
        assert_eq!(result, expected_result);
    }
