...
impl FeesCalculator for IntOnEosEosTxInfos {
    fn get_fees(&self, dictionary: &EosEthTokenDictionary) -> Result<Vec<(EthAddress, U256)>> {
        debug!("Calculating fees in `IntOnEosEosTxInfos`...");
        self.iter()
...
impl FeeCalculator for IntOnEosEosTxInfo {
    fn get_amount(&self) -> U256 {
        info!(
            "✔ Getting token amount in `IntOnEosEosTxInfo` of {}",
            self.token_amount
        );
        self.token_amount
    }
    ...
    fn get_eth_token_address(&self) -> EthAddress {
        debug!(
            "Getting EOS token address in `IntOnEosEosTxInfo` of {}",
    ...
    fn subtract_amount(&self, subtrahend: U256) -> Result<Self> {
        if subtrahend >= self.token_amount {
            Err("Cannot subtract amount from `IntOnEosEosTxInfo`: subtrahend too large!".into())
        } else {
            let new_amount = self.token_amount - subtrahend;
            debug!(
                "Subtracting {} from {} to get final amount of {} in `IntOnEosEosTxInfo`!",
        ...
        Ok(Self::new(
            Self::get_supported_erc20_peg_in_logs_from_receipt(receipt, token_dictionary)
                ...
                .map(|log| {
                    ...
                    let peg_in_info = IntOnEosEosTxInfo {
    ...
    fn filter_eth_sub_mat_for_supported_peg_ins(
        ...
        let filtered_receipts = EthReceipts::new(
            submission_material
                ...
                .iter()
                .filter(|receipt| {
                    IntOnEosEosTxInfos::receipt_contains_supported_erc20_peg_in(receipt, token_dictionary)
                })
                .cloned()
                .collect(),
        );
...
pub fn filter_submission_material_for_peg_in_events_in_state<D: DatabaseInterface>(
    ...
    state
        ...
        .and_then(|filtered_submission_material| {
            IntOnEosEosTxInfos::filter_eth_sub_mat_for_supported_peg_ins(
...
mod tests {
    ...
    fn get_sample_zero_eos_asset_peg_in_info() -> IntOnEosEosTxInfo {
        let user_data = vec![0xde, 0xca, 0xff];
        IntOnEosEosTxInfo::new(
    ...
    fn should_filter_out_zero_eos_asset_peg_ins() {
        ...
        let peg_ins = IntOnEosEosTxInfos::new(vec![get_sample_zero_eos_asset_peg_in_info()]);
    ...
    fn log_is_supported_erc20_peg_in_should_be_true_if_supported() {
        ...
        let result = IntOnEosEosTxInfos::is_log_supported_erc20_peg_in(&log, &token_dictionary).unwrap();
        assert!(result);
    }
    ...
    fn erc20_log_with_peg_in_should_be_erc20_log_with_peg_in() {
        let log = get_sample_log_with_erc20_peg_in_event().unwrap();
        let result = IntOnEosEosTxInfos::is_log_erc20_peg_in(&log).unwrap();
        assert!(result);
    }
    ...
    fn should_return_true_if_receipt_contains_log_with_erc20_peg_in() {
        ...
        let result = IntOnEosEosTxInfos::receipt_contains_supported_erc20_peg_in(&receipt, &dictionary);
        assert!(result);
    }
    ...
    fn should_get_supported_erc20_peg_in_logs() {
        ...
        let dictionary = get_sample_eos_eth_token_dictionary();
        let receipt = get_sample_receipt_with_erc20_peg_in_event().unwrap();
        let result = IntOnEosEosTxInfos::get_supported_erc20_peg_in_logs_from_receipt(&receipt, &dictionary);
    ...
    fn should_get_get_erc20_redeem_infos_from_receipt() {
        ...
        let result = IntOnEosEosTxInfos::from_eth_receipt(&receipt, &token_dictionary, &origin_chain_id).unwrap();
    ...
    fn should_filter_submission_material_for_receipts_containing_supported_erc20_peg_ins() {
        ...
        let filtered_submission_material =
            IntOnEosEosTxInfos::filter_eth_sub_mat_for_supported_peg_ins(&submission_material, &token_dictionary)
                .unwrap();
    ...
    fn should_get_erc20_on_eos_peg_in_infos() {
        ...
        let result =
            IntOnEosEosTxInfos::from_submission_material(&submission_material, &token_dictionary, &origin_chain_id)
                .unwrap();
        assert_eq!(result.len(), expected_num_results);
        assert_eq!(result, expected_result);
    }
    ...
    fn should_get_eos_signed_txs_from_peg_in_infos() {
        let user_data = vec![];
        let info = IntOnEosEosTxInfo::new(
            U256::from_dec_str("1337").unwrap(),
        ...
        let infos = IntOnEosEosTxInfos::new(vec![info]);
