...
impl IntOnAlgoIntTxInfos {
    ...
    fn from_algo_tx(
        ...
        let tx_info = IntOnAlgoIntTxInfo {
            ...
            evm_token_address: dictionary.get_evm_address_from_asset_id(asset_id)?,
    ...
    pub fn from_algo_txs(
        ...
        Ok(Self::new(
            txs.iter()
                .map(|tx| Self::from_algo_tx(tx, dictionary, router_address, genesis_hash))
        ...
        .and_then(|block| match block.transactions {
            ...
            Some(txs) => {
                ...
                state
                    ...
                    .and_then(|tx_infos| state.add_int_on_algo_int_tx_infos(tx_infos))
            },
        })
}
