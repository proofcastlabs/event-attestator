...
impl FeesCalculator for EosOnIntIntTxInfos {
    fn get_fees(&self, dictionary: &EosEthTokenDictionary) -> Result<Vec<(EthAddress, U256)>> {
        debug!("Calculating fees in `EosOnIntIntTxInfos`...");
        self.iter()
...
impl EosOnIntIntTxInfos {
            ...
            material
                ...
                .map(|receipt| {
                    receipt
                        ...
                        .iter()
                        .map(|log| {
                            EosOnIntIntTxInfo::from_eth_log(
                                log,
                ...
                .collect::<Result<Vec<Vec<EosOnIntIntTxInfo>>>>()?
                .concat(),
        ))
    }
    ...
    pub fn to_eos_signed_txs(
        ...
        info!("✔ Signing {} EOS txs from `EosOnIntIntTxInfos`...", self.len());
        Ok(EosSignedTransactions::new(
            self.iter()
                .enumerate()
                .map(|(i, tx_info)| {
                    info!("✔ Signing EOS tx from `EosOnIntIntTxInfo`: {:?}", tx_info);
    ...
    fn filter_out_those_with_zero_eos_asset_amount(&self, dictionary: &EosEthTokenDictionary) -> Self {
        info!("✔ Filtering out `EosOnIntIntTxInfos` if they have a zero EOS asset amount...");
        Self::new(
            self.iter()
                .filter(|tx_info| {
...
impl FeeCalculator for EosOnIntIntTxInfo {
    fn get_amount(&self) -> U256 {
        info!("✔ Getting token amount in `EosOnIntIntTxInfo` of {}", self.token_amount);
        self.token_amount
    }

    fn get_eth_token_address(&self) -> EthAddress {
        debug!(
            "Getting EOS token address in `EosOnIntIntTxInfo` of {}",
...
impl EosOnIntIntTxInfo {
    pub fn from_eth_log(
        ...
        info!("✔ Parsing `EosOnIntIntTxInfo` from ETH log...");
    ...
    pub fn to_eos_signed_tx(
        ...
        let metadata = if self.user_data.is_empty() {
            ...
            info!("✔ Wrapping `user_data` in metadata for `EosOnIntIntTxInfo`...");
...
pub fn maybe_filter_out_eth_tx_info_with_value_too_low_in_state<D: DatabaseInterface>(
    ...
    info!("✔ Maybe filtering `EosOnIntIntTxInfos`...");
...
pub fn maybe_sign_eos_txs_and_add_to_eth_state<D: DatabaseInterface>(state: EthState<D>) -> Result<EthState<D>> {
    info!("✔ Maybe signing `EosOnIntIntTxInfos`...");
...
mod tests {
    ...
    fn get_sample_eos_on_eth_eth_tx_infos() -> EosOnIntIntTxInfos {
        EosOnIntIntTxInfos::from_eth_submission_material(
    ...
    fn get_sample_eos_on_eth_eth_tx_info() -> EosOnIntIntTxInfo {
    ...
    fn should_filter_out_zero_eth_amounts() {
        ...
        let result_before = EosOnIntIntTxInfos::from_eth_submission_material_without_filtering(
            &submission_material,
            &dictionary,
            &origin_chain_id,
        )
    ...
    fn should_default_to_safe_address_when_signing_tx_with_bad_eos_account_name_in_submission_material() {
        ...
        let origin_chain_id = EthChainId::Rinkeby;
        let tx_infos =
            EosOnIntIntTxInfos::from_eth_submission_material(&submission_material, &token_dictionary, &origin_chain_id)
                .unwrap();
        let ref_block_num = 1;
    ...
    fn same_param_tx_infos_should_not_create_same_signatures() {
        ...
        let origin_chain_id = EthChainId::Rinkeby;
        let tx_infos =
            EosOnIntIntTxInfos::from_eth_submission_material(&submission_material, &dictionary, &origin_chain_id)
                .unwrap();
        let ref_block_num = 1;
        let ref_block_prefix = 2;
        let chain_id = EosChainId::EosMainnet;
    ...
    fn should_subtract_fees_from_eos_on_eth_eth_tx_infos() {
        ...
        let expected_result = EosOnIntIntTxInfos::new(vec![get_sample_eos_on_eth_eth_tx_info()
