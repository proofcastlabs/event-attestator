...
mod tests {
    ...
    fn should_submit_evm_rinkeby_block_with_peg_out_successfully() {
        ...
        reset_eth_chain(
            parse_eth_submission_material_and_put_in_state(
                &read_to_string("src/v2/int_on_evm/test_utils/before-peg-out-1-block.json").unwrap(),
                EthState::init(&db),
            )
            .unwrap(),
            confirmations,
        ...
        .unwrap();
        let submission_string = read_to_string("src/v2/int_on_evm/test_utils/peg-out-block-1.json").unwrap();
    ...
    fn should_submit_evm_goerli_block_with_peg_out_successfully() {
        ...
        // NOTE: Finally, submit the block containing the peg in....
        let submission_string = read_to_string("src/v2/int_on_evm/test_utils/goerli-core-peg-out-block.json").unwrap();
        let output = submit_evm_block_to_core(&db, &submission_string).unwrap();
        let expected_result_json = json!({
