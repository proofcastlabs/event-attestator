    ...
    where D: DatabaseInterface
{
    ...
    get_eth_tail_block_from_db(db)
        .map(|db_tail_block| db_tail_block.block.number < calculated_tail_block.block.number)
}

pub fn maybe_update_eth_tail_block_hash<D>(state: EthState<D>) -> Result<EthState<D>> where D: DatabaseInterface {
                ...
                Some(ancestor_block) => {
                    ...
                    match does_tail_block_require_updating(&state.db, &ancestor_block)? {
                        ...
                        true => {
                            ...
                            put_eth_tail_block_hash_in_db(&state.db, &ancestor_block.block.hash).and(Ok(state))
                        }
                    }
                }
            }
