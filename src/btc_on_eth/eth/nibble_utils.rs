...
mod tests {
    ...
    fn should_replace_byte_in_nibbles_correctly() {
        ...
        updated_nibbles
            ...
            .map(|(i, byte)| match i == byte_index {
                true => assert!(byte == &replacement_byte),
                false => assert!(byte == &original_bytes[i]),
            })
            .for_each(drop);
    }
    ...
    fn should_get_common_prefix_nibbles_recursively_when_both_not_offset() {
        ...
        let (result_prefix, result_shorter, result_longer) = get_common_prefix_nibbles_recursively(
            shorter_nibbles,
            longer_nibbles,
            EMPTY_NIBBLES,
        ).unwrap();
    ...
    fn should_get_common_prefix_when_no_common_prefix_and_neither_offset() {
        ...
        let (result_prefix, result_1, result_2) = get_common_prefix_nibbles_recursively(
            nibbles_1,
            nibbles_2,
            EMPTY_NIBBLES,
        ).unwrap();
