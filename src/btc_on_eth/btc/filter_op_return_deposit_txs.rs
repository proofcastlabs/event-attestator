...
fn sig_script_contains_pub_key(script_sig: &BtcScript, btc_pub_key_slice: &[u8]) -> bool {
    hex::encode(btc_serialize(script_sig)).contains(&hex::encode(btc_pub_key_slice))
}

fn tx_has_input_locked_to_pub_key(tx: &BtcTransaction, btc_pub_key_slice: &[u8]) -> bool {
    tx
...
mod tests {
    ...
    fn should_not_filter_out_external_p2pkh_deposits() {
        ...
        let filtered_txs =
            filter_txs_for_op_return_deposits(&get_sample_btc_private_key(), &block_and_id.block.txdata).unwrap();
