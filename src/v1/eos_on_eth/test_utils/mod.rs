...
fn get_sample_eos_submission_material_string_n(n: usize) -> Result<String> {
    let path = match n {
        1 => Ok("src/v1/eos_on_eth/test_utils/eos-submission-material-1.json"),
...
fn get_sample_eth_submission_material_string_n(n: usize) -> Result<String> {
    let path = format!("src/v1/eos_on_eth/test_utils/eth-submission-material-{}.json", n);
    match Path::new(&path).exists() {
        true => Ok(read_to_string(path)?),
...
pub fn get_eth_submission_material_with_bad_eos_account_name() -> EthSubmissionMaterial {
    EthSubmissionMaterial::from_str(
        &read_to_string("src/v1/eos_on_eth/test_utils/eos-bad-account-name-submission-material.json").unwrap(),
    )
    .unwrap()
}
...
pub fn get_eth_submission_material_with_two_peg_ins() -> EthSubmissionMaterial {
    EthSubmissionMaterial::from_str(
        &read_to_string("src/v1/eos_on_eth/test_utils/eth-submission-material-with-two-eos-on-eth-pegins.json")
            .unwrap(),
    )
    .unwrap()
}
