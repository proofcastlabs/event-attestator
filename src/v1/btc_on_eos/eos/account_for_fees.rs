...
pub fn maybe_account_for_fees<D: DatabaseInterface>(state: EosState<D>) -> Result<EosState<D>> {
    ...
    if DISABLE_FEES {
        ...
        Ok(state)
    } else if state.btc_on_eos_btc_tx_infos.is_empty() {
        ...
        .and_then(|updated_btc_tx_info| state.replace_btc_on_eos_btc_tx_infos(updated_btc_tx_info))
    }
}
...
mod tests {
    use super::*;
    use crate::{btc_on_eos::test_utils::get_sample_btc_tx_infos, test_utils::get_test_database};
