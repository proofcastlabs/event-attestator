...
pub fn check_core_is_initialized<D: DatabaseInterface>(btc_db_utils: &BtcDbUtils<D>, db: &D) -> Result<()> {
    check_btc_core_is_initialized(btc_db_utils).and_then(|_| check_eos_core_is_initialized(db))
}

pub fn check_core_is_initialized_and_return_eos_state<D: DatabaseInterface>(state: EosState<D>) -> Result<EosState<D>> {
    check_core_is_initialized(&state.btc_db_utils, state.db).and(Ok(state))
}
