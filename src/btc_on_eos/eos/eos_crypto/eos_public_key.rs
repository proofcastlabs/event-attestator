...
impl EosPublicKey {
    ...
    pub fn to_eos_format(&self) -> String {
        ...
        let mut public_key: [u8; PUBLIC_KEY_WITH_CHECKSUM_SIZE] =
            [0u8; PUBLIC_KEY_WITH_CHECKSUM_SIZE];
        public_key[..PUBLIC_KEY_SIZE]
            .copy_from_slice(self.to_bytes().as_ref());
        public_key[PUBLIC_KEY_SIZE..]
            .copy_from_slice(&h160.take()[..PUBLIC_KEY_CHECKSUM_SIZE]);
    ...
    pub fn recover_from_digest(
        ...
        Self::from_slice(
            &Secp256k1::new()
                .recover(&digest, &recoverable_signature.0)?
                .serialize()
        )
    }
}
...
mod test {
    ...
    fn should_verify_good_signature() {
        ...
        if let Err(e) = public_key
            .verify_signature(
                &get_sample_message_to_sign_bytes(),
                &signature
            ) {
    ...
    fn should_fail_to_verify_incorrect_signature() {
        ...
        let public_key = get_sample_eos_public_key();
        let incorrect_signature = "SIG_K1_KXBtTkGyMHkMCjNzephUVGwGGjGyrQjRg9fiLhqFAZvVCgNMGN9gvmnawM86zKzVvB5yVKiT4NYnaNGxi77m6CvGfjudnT";
        let signature = EosSignature::from_str(incorrect_signature)
            .unwrap();
    ...
    fn should_sha256_hash_message_correctly() {
        ...
        let expected_result_slice = hex::decode(expected_result_hash)
            .unwrap();
        let expected_result = Message::from_slice(&expected_result_slice)
            .unwrap();
    ...
    fn should_recover_eos_public_key_from_hashed_message_and_signature() {
        ...
        let hashed_message = sha256_hash_message_bytes(&message_bytes)
            .unwrap();
        let private_key = get_sample_eos_private_key();
        let public_key = get_sample_eos_public_key();
    ...
    fn should_get_public_key_from_slice_correctly() {
        let result = EosPublicKey::from_slice(
            &get_sample_eos_public_key_bytes()
        ).unwrap();
