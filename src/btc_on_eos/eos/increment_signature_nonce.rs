use crate::{
    ...
    chains::eos::increment_signature_nonce::increment_signature_nonce,
    btc_on_eos::{
    ...
    where D: DatabaseInterface
{
    ...
    match num_txs {
        ...
        _ => {
            increment_signature_nonce(&state.db, get_btc_account_nonce_from_db(&state.db)?, *num_txs as u64)
                .and(Ok(state))
        }
    }
}
