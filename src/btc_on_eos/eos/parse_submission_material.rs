    ...
    Ok(
        EosBlockHeader::new(
            ...
            match &eos_block_header_json.header_extension {
                None => vec![],
                Some(hex_strings) => convert_hex_strings_to_extensions(&hex_strings)?
            },
        )
    )
}
...
mod tests {
    ...
    fn should_parse_j3_block_with_new_producers_schedule_field_correctly() {
        ...
        let block_json: EosBlockHeaderJson = match serde_json::from_str(&block_str) {
            Ok(result) => result,
            Err(e) => panic!("Error converting J3 block w/ new producers to json: {}", e)
        };
    ...
    fn should_parse_mainnet_block_with_new_producers_field_correctly() {
        ...
        let block_json: EosBlockHeaderJson = match serde_json::from_str(&block_str) {
            Ok(result) => result,
            Err(e) => panic!("Error converting mainnet block w/ new producers to json: {}", e)
        };
        if let Err(e) = parse_eos_block_header_from_json(&block_json) {
