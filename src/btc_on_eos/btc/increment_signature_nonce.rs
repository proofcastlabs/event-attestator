...
{
    debug!("✔ Incrementing signature nonce from {} to {}", current_nonce, num_signatures);
    put_eos_account_nonce_in_db(db, current_nonce + num_signatures)
}

pub fn maybe_increment_signature_nonce<D: DatabaseInterface>(state: BtcState<D>) -> Result<BtcState<D>> {
    ...
    match num_txs {
        ...
        _ => increment_signature_nonce(&state.db, get_eos_account_nonce_from_db(&state.db)?, *num_txs as u64)
            .and(Ok(state))
    }
}
