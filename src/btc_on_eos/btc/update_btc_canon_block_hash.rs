    ...
    get_btc_latest_block_from_db(&state.db)
        ...
        .and_then(|maybe_ancester_block_and_id|
            match maybe_ancester_block_and_id {
                None => {
                    info!("✔ No {}th ancestor block in db yet ∴  not updating canon block hash!", canon_to_tip_length);
                    Ok(state)
                }
                Some(ancestor_block) => {
                    ...
                    match does_canon_block_require_updating(&state.db, &ancestor_block)? {
                        ...
                        true => {
                            ...
                            put_btc_canon_block_hash_in_db(&state.db, &ancestor_block.id).and(Ok(state))
                        }
                    }
                }
            }
