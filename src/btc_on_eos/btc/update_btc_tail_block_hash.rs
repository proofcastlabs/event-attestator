...
pub fn maybe_update_btc_tail_block_hash<D: DatabaseInterface>(state: BtcState<D>) -> Result<BtcState<D>> {
    ...
    get_btc_latest_block_from_db(&state.db)
        .map(|latest_btc_block| {
            info!("✔ Searching for tail block {} blocks back from tip...", canon_to_tip_length + BTC_TAIL_LENGTH);
            maybe_get_nth_ancestor_btc_block_and_id(
                &state.db,
                &latest_btc_block.id,
        ...
        .and_then(|maybe_ancester_block_and_id|
            match maybe_ancester_block_and_id {
                None => {
                    info!("✔ No {}th ancestor block in db yet ∴ not updating tail block hash!", canon_to_tip_length);
                    Ok(state)
                }
                Some(ancestor_block) => {
                    ...
                    match does_tail_block_require_updating(&state.db, &ancestor_block)? {
                        ...
                        true => {
                            ...
                            put_btc_tail_block_hash_in_db(&state.db, &ancestor_block.id).and(Ok(state))
                        }
                    }
                }
            }
