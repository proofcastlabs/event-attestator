...
use crate::eos_producer_key::{EosKey, EosKeysAndThreshold, EosProducerKeyV1, EosProducerKeyV2};
...
impl EosProducerScheduleJsonV1 {
    pub fn from(schedule_string: &str) -> common::Result<Self> {
...
impl EosProducerScheduleV2 {
    pub fn schedule_hash(&self) -> common::Result<Checksum256> {
...
impl EosProducerScheduleV1 {
    ...
    pub fn from_schedule_json(json: &EosProducerScheduleJsonV1) -> common::Result<Self> {
        Ok(Self {
            version: json.version,
...
) -> common::Result<Vec<EosProducerKeyV2>> {
    json.iter()
        .map(convert_full_producer_key_json_to_v2_producer_key)
        .collect()
}

fn convert_full_producer_key_json_to_v2_producer_key(json: &FullProducerKeyJsonV2) -> common::Result<EosProducerKeyV2> {
    Ok(EosProducerKeyV2 {
...
) -> common::Result<Vec<EosProducerKeyV1>> {
    json.iter()
        .map(convert_v1_producer_key_json_to_v1_producer_key)
        .collect()
}

fn convert_v1_producer_key_json_to_v1_producer_key(json: &ProducerKeyJsonV1) -> common::Result<EosProducerKeyV1> {
...
fn convert_authority_json_to_eos_keys_and_threshold(json: &AuthorityJson) -> common::Result<EosKeysAndThreshold> {
...
fn convert_keys_json_to_vec_of_eos_keys(keys_json: &[ProducerKeyJsonV2]) -> common::Result<Vec<EosKey>> {
