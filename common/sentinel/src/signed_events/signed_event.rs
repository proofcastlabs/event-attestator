...
impl SignedEvent {
    pub(super) fn new(
        ...
        let mut signed_event = Self {
            ...
            chain_id: metadata_chain_id.to_eth_chain_id()?,
            log,
            transaction_hash,
            block_hash,
            encoded_event: None,
        ...
        signed_event.encoded_event = Some(hex::encode(encoded_event));
    ...
    fn encode(&self) -> Bytes {
        ...
        [address, sha256_hash_bytes(&topics), self.log.data.to_vec()].concat()
    }

    fn calculate_event_id(&self) -> Result<EventId, EventIdError> {
        let encoded_event = self.encoded_event.as_ref().ok_or(EventIdError::EncodedEventIsNone)?;
        let event_id = [
            self.version.as_bytes(),
            &[self.protocol.into()],
            &left_pad_bytes_with_zeroes(&self.chain_id.to_bytes()?, CHAIN_ID_PADDING),
            self.block_hash.as_bytes(),
            self.transaction_hash.as_bytes(),
            &hex::decode(encoded_event)?,
        ]
        .concat();
...
mod tests {
    ...
    fn should_create_a_signed_event_correctly() {
        ...
        let data = hex::decode("").unwrap();
        ...
        let result = SignedEvent::new(metadata_chain_id, log, transaction_hash, block_hash, &pk, merkle_proof).unwrap();
