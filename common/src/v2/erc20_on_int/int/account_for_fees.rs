...
pub fn update_accrued_fees_in_dictionary_and_return_state<D: DatabaseInterface>(
    ...
    if DISABLE_FEES {
        ...
        let tx_infos = Erc20OnIntEthTxInfos::from_bytes(&state.tx_infos)?;
        dictionary
            .increment_accrued_fees_and_save_in_db(state.db, tx_infos.get_fees(&dictionary)?)
            .and(Ok(state))
    }
}
...
mod tests {
    use super::*;
    use crate::erc20_on_int::test_utils::get_sample_eth_tx_info;

    #[test]
    fn should_calculate_eth_on_evm_eth_tx_info_fee() {
        let fee_basis_points = 25;
        let info = get_sample_eth_tx_info();
